...
# --- 웹페이지 기본 설정 및 🖨️ 인쇄용 CSS 숨김 처리 ---
st.set_page_config(page_title="간호학원 만족도 조사 앱", page_icon="📊", layout="wide")

# Ctrl+P 인쇄 시 불필요한 UI(업로더, 버튼, 헤더 등)를 마법처럼 숨기는 CSS 코드
st.markdown("""
    <style>
    @media print {
        /* 상단 헤더, 여백, 사이드바 숨기기 */
        ...
        /* 파일 업로더, 버튼, 구분선 숨기기 */
        .stFileUploader, .stButton, .stDivider {
            display: none !important;
        }
        /* 인쇄 시 화면을 넓게 쓰도록 패딩 제거 */
...
def generate_excel(final_df):
    ...
    with pd.ExcelWriter(buffer, engine='openpyxl') as writer:
        ...
        # ✨ 엑셀 칸 높이 알고리즘 대폭 강화 (글씨 짤림 완벽 방지) ✨
        for r_idx, row in enumerate(ws_raw.iter_rows(min_row=2), start=2):
            ...
            # 15글자마다 줄바꿈이 일어난다고 아주 넉넉하게 가정 (기존 20 -> 15)
            lines_imp = improve_text.count('\n') + (len(improve_text) // 15) + 1
            lines_rev = review_text.count('\n') + (len(review_text) // 15) + 1
            
            # 한 줄당 부여하는 높이도 25로 크게 증가 (기존 18 -> 25)
            ws_raw.row_dimensions[r_idx].height = max(45, max(lines_imp, lines_rev) * 25)
        ...
        def write_text_section(title, column_name, start_row):
            ...
            if len(items) == 0:
                ...
                for item in items:
                    ...
                    ws_sum.row_dimensions[current_row].height = max(30, lines * 22)
                    current_row += 1
                current_row += 1
            return current_row
...
if st.button("🚀 데이터 분석 및 엑셀 다운로드 파일 생성", width='stretch'):
        ...
        with st.spinner('데이터를 분석하고 웹 대시보드를 생성하고 있습니다...'):
            ...
            if not result_df.empty:
                ...
                with tab1:
                    ...
                    # ✨ 요약표도 데이터 길이에 맞춰 스크롤 없이 다 보이도록 높이 지정
                    summary_height = (len(table_data) + 1) * 36 + 40
                    st.dataframe(pd.DataFrame(table_data).astype(str), width='stretch', height=summary_height, hide_index=True)
                
                with tab2:
                    st.subheader("📋 전체 원본 데이터 확인")
                    st.write("가로로 스크롤하여 모든 학생들의 개별 응답을 확인할 수 있습니다.")
                    
                    # ✨ 데이터 행(학생 수) 개수를 파악하여 표 세로 길이를 쫙 늘려줌 (내부 스크롤 제거)
                    df_height = (len(result_df) + 1) * 36 + 40
                    st.dataframe(result_df.astype(str), width='stretch', height=df_height, hide_index=True)
            else:
